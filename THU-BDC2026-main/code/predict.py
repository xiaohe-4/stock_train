...
def main():
	...
	train_medians_path = os.path.join(config['output_dir'], 'train_medians.pkl')
	if os.path.exists(train_medians_path):
	...
	score_range = top5_scores.max() - top5_scores.min()
	if score_range > 1e-6:
		exp_scores = np.exp((top5_scores - top5_scores.max()) * 2.0)
